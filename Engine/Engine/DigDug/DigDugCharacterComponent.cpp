...
void dae::DigDugCharacterComponent::UpdateArrowCollision()
{
	if (!m_IsRegistered)
	{
		m_CollisionIndex = Locator::GetCollisionComponent()->GetComponent<CollisionComponent>()->AddCollider(Tag{ "PlayerArrow" }, Rectf{ m_PlayerPos.x + 48.0f,m_PlayerPos.y, 96.0f,48.0f });
		m_IsRegistered = true;
	}
	else if (m_IsPumping)
	{
		...
		case dae::Direction::right:
			Locator::GetCollisionComponent()->GetComponent<CollisionComponent>()->UpdateCollision(m_CollisionIndex, Rectf{ m_PlayerPos.x + 48.0f,m_PlayerPos.y, 96.0f,48.0f });
			break;
		case dae::Direction::down:
			Locator::GetCollisionComponent()->GetComponent<CollisionComponent>()->UpdateCollision(m_CollisionIndex, Rectf{ m_PlayerPos.x,m_PlayerPos.y + 48.0f, 48,96.0f });
			break;
		case dae::Direction::left:
			Locator::GetCollisionComponent()->GetComponent<CollisionComponent>()->UpdateCollision(m_CollisionIndex, Rectf{ m_PlayerPos.x - 96.0f,m_PlayerPos.y, 96.0f,48.0f });
			break;
		case dae::Direction::up:
			Locator::GetCollisionComponent()->GetComponent<CollisionComponent>()->UpdateCollision(m_CollisionIndex, Rectf{ m_PlayerPos.x,m_PlayerPos.y - 96.0f, 48.0f,96.0f });
			break;
		}
	}
	else
...
{
	auto isHit = Locator::GetCollisionComponent()->GetComponent<CollisionComponent>()->CheckCollision(Tag{ "Enemy" }, Rectf{ m_PlayerPos.x, m_PlayerPos.y,48.0f,48.0f });
	if (isHit && !m_IsDead)
	{
		m_IsDead = true;
...
{
	auto isHit = Locator::GetCollisionComponent()->GetComponent<CollisionComponent>()->CheckCollision(Tag{ "Stone" }, Rectf{ m_PlayerPos.x, m_PlayerPos.y,48.0f,48.0f });
	if (isHit && !m_IsDead)
	{
		m_IsDead = true;
		if (GetGameObject()->GetComponent<HealthComponent>()->DecreaseHealth() == 0)
